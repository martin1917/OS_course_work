...
struct sembuf lock_res_w = {1, -1, 0};
struct sembuf rel_res_w = {1, 1, 0};
struct sembuf lock_res_r = {0, -1, 0};
struct sembuf rel_res_r = {0, 1, 0};
...
int main()
{
    ...
    if ((shmid_1 = shmget(SHM_KEY_2, sizeof(mailbox_t), IPC_CREAT | 0666)) < 0) {
        perror("shmget_1");
        exit(1);
    }
    ...
    if ((shmid_2 = shmget(SHM_KEY_1, sizeof(mailbox_t), IPC_CREAT | 0666)) < 0) {
        perror("shmget_2");
        exit(1);
    }
